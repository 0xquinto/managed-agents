...
**Important:** `platform.claude.com` docs pages are JS-rendered SPAs. `WebFetch` and `curl` return empty shells. You MUST use `mcp__exa__crawling_exa` (or the `get-code-context-exa` skill as fallback) for per-subcommand content. `WebFetch` is only usable against `sitemap.xml`, which is plain XML.
...
1. `WebFetch` `https://platform.claude.com/sitemap.xml` (plain XML — WebFetch works here; do NOT use Exa crawl on the sitemap).
2. Extract all `<loc>` entries whose path matches `/docs/en/api/cli/beta/<path>`. Keep only **leaf** URLs (paths that don't have children also present — e.g., keep `/cli/beta/agents/create` but drop the domain-index `/cli/beta/agents` when its children are present).
...
- **Coverage mode:** `WebFetch` on `sitemap.xml` only. The sitemap is plain XML, not a SPA — use WebFetch, not Exa crawl.
...
If `mcp__exa__crawling_exa` fails on the section-mode URL AND the `get-code-context-exa` skill returns nothing authoritative (or for coverage mode, `WebFetch` on the sitemap fails), return:
