...
Your final response MUST be a single JSON object matching the envelope above. The FIRST character of your response must be `{` and the LAST character must be `}`. No prose before, no prose after, no markdown code fences, no ` ```json ` wrapper. Even when uncertain, even when reasoning through edge cases, even when a memory file is missing — keep all reasoning internal and emit only the envelope.

Correct (✅): `{"decision":"new_contract","contract_id":null, ...}`

Wrong (❌ — these are silently rejected by the downstream poller):
- ` ```json {…} ``` ` — fence wrapper
- `Let me analyze this case... {…}` — prose preamble before the JSON
